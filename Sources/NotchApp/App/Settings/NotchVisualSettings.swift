...
final class NotchVisualSettings: ObservableObject {
    ...
    init(defaults: UserDefaults = .standard) {
        ...
        lineColor = Color(rgbaHex: defaults.string(forKey: Key.lineColor) ?? "8FCBFFFF")
        lineGradientColor = Color(rgbaHex: defaults.string(forKey: Key.lineGradientColor) ?? "8FCBFFFF")
        ...
        compactHeight = min(
            ...
            max(
                NotchLayout.compactHeightRange.lowerBound,
                CGFloat(storedCompactHeight ?? Double(NotchLayout.defaultCompactHeight))
            )
        )
    }
}

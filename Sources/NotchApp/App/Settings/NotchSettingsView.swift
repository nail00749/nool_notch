...
struct NotchSettingsView: View {
                ...
                VStack(alignment: .leading, spacing: 10) {
                    ...
                    HStack(spacing: 8) {
                        ...
                        Text("Сейчас: \(displaySettings.activeDisplay?.name ?? "дисплей не определён")")
                            .foregroundStyle(NotchPalette.text.opacity(0.68))
                            .lineLimit(1)
                        Spacer(minLength: 8)
                    ...
                    if displaySettings.usesPerDisplayCompactHeight {
                        ForEach(displaySettings.connectedDisplays) { display in
                            ...
                            if display.id != displaySettings.connectedDisplays.last?.id {
                                Divider().overlay(NotchPalette.text.opacity(0.06))
                            }
                        }
                    } else {
                ...
                Image(systemName: display.isBuiltIn ? "laptopcomputer" : "display")
                    .foregroundStyle(
                        display.id == displaySettings.activeDisplayID
                            ? NotchPalette.accent : NotchPalette.secondary
                    )
                    .frame(width: 18)
                Text(display.name)
                    .lineLimit(1)
    ...
    private var updateStatusContent: some View {
        ...
        case .idle, .checking:
            HStack(spacing: 8) {
                ProgressView().controlSize(.small).tint(NotchPalette.accent)
    ...
    private func panelConfigurationRow(_ panel: PanelID) -> some View {
        ...
        return HStack(spacing: 8) {
            Image(systemName: panel.iconName)
                ...
                .foregroundStyle(isVisible ? NotchPalette.accent : NotchPalette.text.opacity(0.28))
                .frame(width: 18)
                    ...
                    if model.compactQuotaDisplayMode == .top {
                        ...
                        HStack(spacing: 9) {
                            QuotaProviderBrandIcon(
                                ...
                                size: 14,
                                color: NotchPalette.text.opacity(0.88)
                            )
                            .frame(width: 24, height: 24)
                            .background(
                                NotchPalette.accent.opacity(0.12),
        ...
        return VStack(alignment: .leading, spacing: 5) {
            HStack(spacing: 8) {
                ...
                QuotaProviderBrandIcon(
                    ...
                    size: 13,
                    color: isVisible ? NotchPalette.text.opacity(0.68) : NotchPalette.text.opacity(0.26)
                )
                .frame(width: 17)
    ...
    private func quotaDisplayModeButton(_ mode: CompactQuotaDisplayMode) -> some View {
        ...
        return Button {
            ...
            .background(
                isSelected ? NotchPalette.accent.opacity(0.12) : Color.black.opacity(0.26),
                in: RoundedRectangle(cornerRadius: 12, style: .continuous)
            )
            .overlay {
                RoundedRectangle(cornerRadius: 12, style: .continuous)
                    .stroke(
                        isSelected ? NotchPalette.accent.opacity(0.42) : NotchPalette.text.opacity(0.06),
                        lineWidth: 0.75
                    )
            }
    ...
    private func quotaStackCornerButton(_ corner: QuotaStackCorner) -> some View {
        ...
        return Button {
            ...
            .background(
                isSelected ? NotchPalette.accent.opacity(0.10) : Color.black.opacity(0.24),
                in: RoundedRectangle(cornerRadius: 10, style: .continuous)
            )
            .overlay {
                RoundedRectangle(cornerRadius: 10, style: .continuous)
                    .stroke(
                        isSelected ? NotchPalette.accent.opacity(0.36) : NotchPalette.text.opacity(0.05),
                        lineWidth: 0.75
                    )
            }
        }
    ...
    private var compactQuotaPreviewColor: Color {
        guard let ratio = model.compactWeeklyRemainingRatio else {
            return NotchPalette.text.opacity(0.34)
        }
    ...
    private func quotaStatusColor(_ connection: ProviderConnectionState?) -> Color {
        ...
        case .unavailable, nil:
            NotchPalette.text.opacity(0.28)
        }
    }
...
private struct DiagnosticRow: View {
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(title).foregroundStyle(NotchPalette.text.opacity(0.45))
            Spacer(minLength: 12)
            Text(value)
                .foregroundStyle(NotchPalette.text.opacity(0.82))
                .lineLimit(1)
        ...
        .overlay(alignment: .bottom) {
            if showsDivider {
                Rectangle().fill(NotchPalette.text.opacity(0.06)).frame(height: 1)
            }
        }
    }
}
...
private struct CodeReviewIntegrationRow: View {
    ...
    private func setupButton(title: String, host: String?) -> some View {
        Button { onSetup(host) } label: {
            ...
            .frame(maxWidth: .infinity, minHeight: 40)
            .background(
                NotchPalette.accent.opacity(0.08),
                in: RoundedRectangle(cornerRadius: 10, style: .continuous)
            )
        }
        .buttonStyle(NotchButtonStyle())

...
final class NotchViewModel: ObservableObject {
    ...
    func recognizeText(_ urls: [URL]) { onRecognizeText?(urls) }
    ...
    var snapshots: [String: QuotaSnapshot] { quotas.snapshots }
    ...
    var calendarState: CalendarLoadState { calendar.calendarState }
    var upcomingMeetingReminder: MeetingReminder? { calendar.upcomingMeetingReminder }
    private var dockMusicVisible = false
    private var dockCalendarEnabled = false
    var calendarEventsByMonth: [CalendarMonthKey: [CalendarEvent]] { calendar.calendarEventsByMonth }
    ...
    var dockCalendarEvents: [CalendarEvent] { calendar.reminderEvents }
    ...
    func snapshot(for providerID: String) -> QuotaSnapshot? { quotas.snapshot(for: providerID) }

    var orderedQuotaProviders: [any QuotaProvider]{ quotas.orderedQuotaProviders }

    var visibleQuotaProviders: [any QuotaProvider]{ quotas.visibleQuotaProviders }

    var compactQuotaProviderName: String { quotas.compactQuotaProviderName }

    var compactWeeklyRemainingRatio: Double? { quotas.compactWeeklyRemainingRatio }

    var shouldEnableQuotaEdgePanel: Bool { quotas.shouldEnableQuotaEdgePanel }

    var shouldEnableQuotaCornerStack: Bool { quotas.shouldEnableQuotaCornerStack }
    ...
    func canBeginAuthentication(for providerID: String) -> Bool { quotas.canBeginAuthentication(for: providerID) }
    ...
    var codeReviewSessions: [AISession] { codeReviews.codeReviewSessions }
    ...
    func refreshCodeReviews() { codeReviews.refreshCodeReviews() }
    ...
    func refresh() { quotas.refresh() }

    func loadCalendarIfNeeded() { calendar.loadCalendarIfNeeded() }

    func refreshCalendar() { calendar.refreshCalendar() }
    ...
    func calendarEvents(for month: Date) -> [CalendarEvent]{ calendar.calendarEvents(for: month) }

    func loadCalendarMonthIfNeeded(for month: Date) { calendar.loadCalendarMonthIfNeeded(for: month) }

    func beginAuthentication(for providerID: String) { quotas.beginAuthentication(for: providerID) }
    ...
    private func updateProviderActivity() {
        ...
        let showsCodeReviews = isExpanded
            ...
            && isShowingSettings == false
        codeReviews.setVisible(showsCodeReviews)
    }

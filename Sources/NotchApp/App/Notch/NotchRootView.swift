    ...
    private var surfaceContent: some View {
        ...
        .background {
            ZStack(alignment: .top) {
                ...
                surfaceShape.fill(NotchPalette.surface)
                    .opacity(model.isExpanded ? 1 : 0)

                // Keep the camera reservation black throughout the transition.
                // Header controls use the two measured wings around this area.
                Color.black
                    .frame(
                        width: layoutMetrics.physicalNotchSize.width,
                        height: layoutMetrics.physicalNotchSize.height
                    )
            }
        }
        .overlay {

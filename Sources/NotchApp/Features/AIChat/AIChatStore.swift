...
final class AIChatStore: ObservableObject {
    ...
    enum QuickQuestionResult: Equatable {
        case sent
        case unavailable(String)
        case cancelled
    }

    /// A launcher query is a new, text-only conversation. Never send an
    /// existing chat's context, unsent draft, or attachments implicitly.
    func sendQuickQuestion(_ text: String) async -> QuickQuestionResult {
        let input = text.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !input.isEmpty, text.count <= 8_000 else {
            return .unavailable("Введите вопрос длиной до 8 000 символов.")
        }
        if !historyLoadFinished { await waitForPersistence() }
        guard !Task.isCancelled else { return .cancelled }
        guard !isStreaming, !isImportingAttachments else {
            return .unavailable("Дождитесь текущего ответа или загрузки вложений в AI-чате.")
        }
        guard !historyLoadFailed else {
            return .unavailable("История AI недоступна. Откройте AI-чат, чтобы проверить её состояние.")
        }
        let providerID = selectedProvider
        let generation = responseGeneration
        refresh(providerID)
        let discoveryGeneration = discoveryGenerations[providerID]
        await discoveryTasks[providerID]?.value
        guard !Task.isCancelled, selectedProvider == providerID,
              responseGeneration == generation,
              discoveryGenerations[providerID] == discoveryGeneration else { return .cancelled }
        guard !isStreaming, !isImportingAttachments else {
            return .unavailable("AI-чат занят. Дождитесь завершения текущего действия.")
        }
        guard isEnabled(providerID), selectedStatus?.isAvailable == true else {
            return .unavailable(selectedStatus?.message ?? "Подключение AI недоступно.")
        }
        guard selectedModels.contains(where: { $0.id == selectedModelID }) else {
            return .unavailable("Выберите доступную модель во вкладке AI.")
        }
        newChat() // Persists the previous conversation, including any draft.
        draft = input
        send()
        return isStreaming ? .sent : .unavailable(errorMessage ?? "Не удалось отправить вопрос.")
    }

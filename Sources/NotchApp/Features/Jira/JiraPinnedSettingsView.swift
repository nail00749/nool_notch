            ...
            SettingsCard(title: "Проекты и доски", icon: "pin") {
                VStack(alignment: .leading, spacing: 10) {
                    ...
                    if model.jiraState.pinned.containers.isEmpty == false {
                        Divider().overlay(NotchPalette.text.opacity(0.08))
                        Text("Закреплено")
        ...
        return Button {
            ...
            HStack(spacing: 9) {
                Image(systemName: container.kind == .board ? "rectangle.split.3x1" : "shippingbox")
                    .foregroundStyle(isPinned ? NotchPalette.accent : NotchPalette.text.opacity(0.48))
                    .frame(width: 18)
                ...
                Image(systemName: isPinned ? "pin.fill" : "pin")
                    .foregroundStyle(isPinned ? NotchPalette.accent : NotchPalette.text.opacity(0.4))
            }
    ...
    private func pinnedIssueRow(_ issue: JiraPinnedIssue, index: Int) -> some View {
        HStack(spacing: 8) {
            VStack(alignment: .leading, spacing: 2) {
                ...
                Text(issue.summary).foregroundStyle(NotchPalette.text.opacity(0.52)).lineLimit(1)
            }

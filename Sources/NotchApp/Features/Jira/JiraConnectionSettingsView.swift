...
struct JiraConnectionSettingsView: View {
        ...
        Button(action: action) {
            Label(title, systemImage: icon)
                ...
                .foregroundStyle(
                    isDestructive
                        ? Color.signalCoral
                        : (isAccent ? NotchPalette.accent : NotchPalette.text.opacity(0.72))
                )
                .frame(maxWidth: .infinity, minHeight: 40)
                .background(
                    NotchPalette.text.opacity(0.09),
    ...
    private func credentialFieldSurface<Content: View>(
        ...
        .frame(height: 42)
        .background(
            NotchPalette.text.opacity(isFocused ? 0.11 : 0.075),
            in: RoundedRectangle(cornerRadius: 12, style: .continuous)
        )
        .overlay {
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .stroke(
                    isFocused ? NotchPalette.accent.opacity(0.82) : NotchPalette.text.opacity(0.13),
                    lineWidth: isFocused ? 1.5 : 1
                )
        }
        .shadow(
            color: isFocused ? NotchPalette.accent.opacity(0.13) : .clear,
            radius: 8
        )

        ...
        VStack(alignment: .leading, spacing: 20) {
            GroupBox("Панель поиска") {
                VStack(alignment: .leading, spacing: 10) {
                    ...
                    if settings.isRecordingShortcut {
                        Text("Используйте ⌘, ⌥ или ⌃. Escape — отмена.").font(.caption).foregroundStyle(.secondary)
                    }
                    if let error = shortcutMessage ?? settings.hotKeyError {
                        Text(error).font(.caption).foregroundStyle(.orange)
                    }
                    HStack {
                        Button("Открыть Launcher", action: open).buttonStyle(.borderedProminent).tint(Color.signalMint)
            ...
            GroupBox("Поиск файлов") {
                VStack(alignment: .leading, spacing: 10) {
                    ...
                    ForEach(settings.folderPaths, id: \.self) { path in
                        HStack(spacing: 8) {
                            Image(systemName: "folder").foregroundStyle(Color.signalMint)
                    ...
                    if settings.folderPaths.isEmpty {
                        Text("Добавьте папку, чтобы включить поиск файлов.").font(.caption).foregroundStyle(.secondary)
                    }
            ...
            GroupBox("История буфера обмена") {
                VStack(alignment: .leading, spacing: 12) {
                    ...
                    HStack {
                        Text("Записей: \(clipboard.items.count)").font(.caption).foregroundStyle(.secondary).monospacedDigit()
                        Spacer()
...
private struct LauncherSettingsGroupStyle: GroupBoxStyle {
    func makeBody(configuration: Configuration) -> some View {
        VStack(alignment: .leading, spacing: 8) {
            configuration.label.font(.system(size: 12, weight: .semibold)).foregroundStyle(.white.opacity(0.9))

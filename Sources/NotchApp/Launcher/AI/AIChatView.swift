...
struct AIChatView: View {
    ...
    var body: some View {
        ...
        .overlay {
            if dropTargeted {
                RoundedRectangle(cornerRadius: 16).strokeBorder(Color.signalMint, style: StrokeStyle(lineWidth: 2, dash: [7]))
                    .background(Color.signalMint.opacity(0.08)).allowsHitTesting(false)
            }
        }
    ...
    private var selectionMenu: some View {
        Menu {
            ...
            Image(systemName: "text.cursor")
                .foregroundStyle(selection.text == nil ? Color.white.opacity(0.45) : Color.signalMint)
    ...
    private func notice(_ message: String, isError: Bool) -> some View {
        ...
        .foregroundStyle(isError ? Color.signalCoral : .white.opacity(0.6))
        .padding(.horizontal, 14).padding(.vertical, 9)
        .background((isError ? Color.signalCoral : .white).opacity(0.06), in: RoundedRectangle(cornerRadius: 10))
    ...
    private var composer: some View {
        VStack(spacing: 8) {
            ...
            .background {
                RoundedRectangle(cornerRadius: 16).fill(Color.white.opacity(composerFocused ? 0.065 : 0.04))
            }
            .overlay {
                RoundedRectangle(cornerRadius: 16)
                    .strokeBorder(composerFocused ? Color.signalMint.opacity(0.32) : Color.white.opacity(0.1))
            }
        ...
        ZStack(alignment: .topLeading) {
            ...
            if store.draft.isEmpty {
                Text("Напишите сообщение…")
                    .font(.system(size: 14)).foregroundStyle(Color.white.opacity(0.34))
    ...
    private var composerActions: some View {
        HStack(spacing: 6) {
            ...
            Text("↵ отправить").foregroundStyle(Color.white.opacity(0.48))
            Text("·  ⇧↵ новая строка").foregroundStyle(Color.white.opacity(0.3))
            Spacer()
            if store.draft.count > 7_000 {
                Text("\(store.draft.count)/8000").monospacedDigit()
                    .foregroundStyle(store.draft.count > 8_000 ? Color.signalCoral : Color.white.opacity(0.5))
            }
            sendButton
        }
        .font(.system(size: 10))
    ...
    private var sendButton: some View {
        Button {
            ...
            Image(systemName: store.isStreaming ? "stop.fill" : "arrow.up")
                ...
                .foregroundStyle(store.isStreaming || store.canSend ? Color.black.opacity(0.85) : Color.white.opacity(0.25))
                .frame(width: 32, height: 32)
                .background {
                    RoundedRectangle(cornerRadius: 10)
                        .fill(store.isStreaming || store.canSend ? Color.signalMint : Color.white.opacity(0.07))
                }
    ...
    private var privacyFooter: some View {
        ...
        .font(.system(size: 10)).foregroundStyle(Color.white.opacity(0.38))
            ...
            if message.role == .user {
                ...
                VStack(alignment: .leading, spacing: 4) {
                        ...
                        if !message.text.isEmpty {
                            ...
                            Button(action: copy) {
                                Image(systemName: copied ? "checkmark" : "doc.on.doc")
                                    .font(.system(size: 12))
                                    .foregroundStyle(copied ? Color.signalMint : .white.opacity(0.45))
                    ...
                    if message.text.isEmpty && message.state == .streaming {
                        Text("Готовлю ответ…").font(.system(size: 13)).foregroundStyle(.white.opacity(0.42))
                            .padding(.top, 5)
                    } else {
                        Text(LocalizedStringKey(message.text))
                            .font(.system(size: 14)).lineSpacing(5).textSelection(.enabled)
                            .fixedSize(horizontal: false, vertical: true)
                            .foregroundStyle(.white.opacity(0.86)).tint(Color.signalMint)
                            .frame(maxWidth: .infinity, alignment: .leading)
                    }
                    if message.state == .interrupted || message.state == .failed {
                        Label("Ответ не завершён", systemImage: "pause.circle")
                            .font(.system(size: 10)).foregroundStyle(.white.opacity(0.4)).padding(.top, 6)
                    }
                }
            }
        }
                ...
                ForEach(attachments) { attachment in
                    HStack(spacing: 8) {
                        if let data = attachment.imageData, let image = NSImage(data: data) {
                            ...
                            Image(systemName: "doc.text").font(.system(size: 18)).foregroundStyle(Color.signalMint)
                    ...
                    .foregroundStyle(.white.opacity(0.85)).padding(6)
...
private struct AIChatQuietButtonStyle: ButtonStyle {
    ...
    func makeBody(configuration: Configuration) -> some View {
        configuration.label
            ...
            .background(.white.opacity(configuration.isPressed ? 0.1 : (hovered ? 0.055 : 0)),
...
private struct AIChatSuggestionStyle: ButtonStyle {
    ...
    func makeBody(configuration: Configuration) -> some View {
        configuration.label
            ...
            .foregroundStyle(.white.opacity(enabled ? (hovered ? 0.9 : 0.65) : 0.3))
            .padding(.horizontal, 13).frame(height: 40)
            .background(.white.opacity(configuration.isPressed ? 0.09 : (hovered ? 0.07 : 0.035)),
                        in: RoundedRectangle(cornerRadius: 10))
...
private struct AIChatComposer: NSViewRepresentable {
    ...
    func makeNSView(context: Context) -> NSScrollView {
        ...
        editor.insertionPointColor = NSColor(Color.signalMint)
